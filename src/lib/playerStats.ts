...
export async function fetchPlayerStats(userId: string): Promise<PlayerStats> {
  ...
  const { data } = await supabase
    .from(TABLE)
    .select('games_played, total_stars, total_points, current_streak, max_streak')
    .eq('user_id', userId)
    .maybeSingle();
 ...
 * of whatever's already saved, and overwrites the streak with the freshly computed local
 * value (lib/storage.ts already handles day-continuity logic) — this is a fetch-then-write,
 * not an atomic increment, which is fine for one browser completing one game at a time. */
